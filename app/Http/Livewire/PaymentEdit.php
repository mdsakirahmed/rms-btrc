...
class PaymentEdit extends Component
{
    ...
    public function mount(\App\Models\Payment $payment)
    {
        if (!auth()->user()->can('master')) {
            if ($this->payment->created_at->diffInDays() > 3 || $payment->created_by != auth()->user()->id) {
                abort(404);
            }
        }
    ...
    {
        if (!auth()->user()->can('master')) {
            if ($this->payment->created_at->diffInDays() > 3 || $this->payment->created_by != auth()->user()->id) {
                abort(404);
            }
        }

        $this->validate([
            'transaction' => 'required|unique:payments,transaction,'.$this->payment->id,
